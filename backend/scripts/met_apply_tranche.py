...
def main():
 ap=argparse.ArgumentParser(); ap.add_argument('--operator',required=True); ap.add_argument('--batch-size',type=int,default=10); ap.add_argument('--limit',type=int); ap.add_argument('--activate',action='store_true'); a=ap.parse_args(); load_dotenv(ROOT/'.env'); import backend.app.db as dbm; dbm=importlib.reload(dbm)
 snap=D/'met_tranche_snapshot_v1.json'; sel=json.loads((D/'met_controlled_selection_v1.json').read_text()); desc=json.loads((D/'met_visual_descriptors_v1.json').read_text()); descriptors={str(x['provider_record_id']):x for x in desc['records']}; payload=json.loads(snap.read_text()); payload['records']=payload['records'][:a.limit] if a.limit else payload['records']; sel['records']=sel['records'][:a.limit] if a.limit else sel['records']
 with dbm.SessionLocal() as db:
  ...
  if not a.activate:
   print(json.dumps({'prepared':len(sel['records']),'activated':0,'recognition_assets':0,'descriptors':len(descriptors)})); return
  # Explicit activation is separate from preparation and only runs after gates pass.
