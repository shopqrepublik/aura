...
def main():
 ap=argparse.ArgumentParser(); ap.add_argument('--limit',type=int,default=20); ap.add_argument('--external-manifest'); a=ap.parse_args(); load_dotenv(ROOT/'.env'); snap=json.loads((D/'met_tranche_snapshot_v1.json').read_text()); man=json.loads((D/'corpus_100_v1/manifest.json').read_text()); rows={str(r['provider_record_id']):r for r in snap['records']}; ready=[r for r in man['records'] if r['status']=='READY'][:a.limit]; candidates=[]
 ...
 if a.external_manifest:
  ext=json.loads(Path(a.external_manifest).read_text())['records'][:a.limit]; ready=[{'provider_record_id':str(i),'files':{'visitor_like':{'path':x['files']['visitor_like']['path']}}} for i,x in enumerate(ext)]
 for i,r in enumerate(ready,1):
  b=(ROOT/r['files']['visitor_like']['path']).read_bytes(); t=time.perf_counter(); result=recognize_with_vision(base64.b64encode(b).decode(),INST,None,candidates,institution_config=cfg); out.append({'provider_record_id':r['provider_record_id'],'expected':None if a.external_manifest else stable_id('artwork',PROVIDER,r['provider_record_id']),'selected':result.get('artwork_id'),'confidence':result.get('confidence',0),'elapsed_s':time.perf_counter()-t,'outcome':result.get('resolution') or result.get('status')}); print(f'{i}/{len(ready)} {out[-1]["outcome"]}',flush=True)
