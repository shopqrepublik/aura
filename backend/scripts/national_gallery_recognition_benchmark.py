...
def main():
    ...
    retry_cases = 0
    for row in results:
        diagnostic = row.get("stage1_diagnostic") or {}
    ...
    summary={"generated_at":datetime.now(timezone.utc).isoformat(),"code_sha":os.getenv("GIT_COMMIT_SHA") or "working-tree","catalog_version":cfg.visitor_catalog_version,"mode":args.mode,"variant":args.variant,"expected_out_of_catalog":args.expect_out_of_catalog,"cases":len(results),"correct_top1":sum(r["correct_top1"] for r in results),"correct_topk":sum(r["correct_topk"] for r in results),"confirmation_required":sum(r["visitor_resolution"]=="CONFIRMATION_REQUIRED" for r in results),"auto_accepted":sum(r["visitor_resolution"]=="AUTO_ACCEPTED" for r in results),"ai_fallback":sum(r["visitor_resolution"]=="AI_UNCATALOGED" for r in results),"unresolved":sum(r["visitor_resolution"]=="UNRESOLVED" for r in results),"incorrect_catalog_match":sum(bool(r["chosen_artwork_id"]) and not r["correct_top1"] for r in results),"confident_incorrect":sum(bool(r["chosen_artwork_id"]) and not r["correct_top1"] and r["confidence"]>=cfg.confidence_auto for r in results),"latency_average_s":statistics.mean(lat) if lat else None,"latency_p50_s":percentile(lat,.5),"latency_p95_s":percentile(lat,.95),"model_calls_average":statistics.mean([row.get("stage_timings_s",{}).get("model_calls",0) for row in results]) if results else 0,"stage1_outcomes":stage1_outcomes,"stage1_retry_cases":retry_cases,"visual_candidate_recall":recall,"stage_timings":stage_summary}
