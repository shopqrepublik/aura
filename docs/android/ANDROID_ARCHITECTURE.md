...
Status: A2.5 WEB PREREQUISITES DEPLOYED AND LIVE-VERIFIED; TWA shell built for device testing; physical-device acceptance still pending (no real hardware run yet).

Audit date: 2026-09-05, updated 2026-09-06 (A2.5 deploy). Canonical Android document. No previous Android/mobile architecture document or Android project was found in the tracked repository.
...
- A2 gate status (superseded by A2.5 below): build-ready for physical-device testing, but not release-ready until the web asset-links route/fingerprint is deployed and verified on both origins, production consent changes are deployed/observed, and the physical matrix is run.

## A2.5 deployment record (2026-09-06)

- **Fixed a real production build break found during this deploy.** `GoogleAnalytics.tsx`'s consent-settings button (`choice !== null`) also matched the SSR-only `undefined` state and called `localeFromPath()` (reads `window.location`) during prerendering, crashing `next build`/`_not-found` with `ReferenceError: window is not defined`. This is why the prior attempted production deploy (`dpl_Cjyom3k4SLpSPf31cCizArz51wBp`, 2026-09-05 17:24 CEST) shows status Error in Vercel. Fixed by treating `undefined` the same as "no decision yet" (render nothing) before checking `!== null`. Verified with a clean local `npm run build` and the actual production build log below.
- **Discovered and fixed a second, independent redirect layer.** `web/proxy.ts` (Next 16 middleware) exempts `/.well-known/assetlinks.json` from the apex→www redirect, but Vercel's **project-level Domain redirect** (`elyio.co` → `www.elyio.co`, 308), configured outside the app via the Vercel dashboard/API, ran before the request ever reached the Next.js app or its middleware and still returned a redirect for every path including the asset-links resource. Fixed by calling the Vercel REST API (`PATCH /v9/projects/:id/domains/elyio.co`) to clear that domain-level `redirect`/`redirectStatusCode`, so `proxy.ts` is now the single source of truth for the canonical-host redirect, with its documented assetlinks.json exception actually taking effect. Confirmed live (see below): `elyio.co/.well-known/assetlinks.json` now returns 200 directly; `elyio.co/en` still 308s to `www.elyio.co/en`.
- Deploy commit: `68bd0dd4bf29904213d614732650ee0868f02c4b` (local `main`, 7 commits ahead of `origin/main`; not pushed to GitHub — this repo's Vercel project deploys via `vercel --prod` CLI upload, not a Git integration build). Vercel production deployment `dpl_BCxj5weLfeBxqJvuMKwbe5VSmH5A`, aliased to `https://www.elyio.co`, `https://elyio.co`, and `https://elyio.vercel.app`.
- `ANDROID_ASSETLINKS_SHA256` production env var set to **two** comma-separated fingerprints: the checked-in debug keystore's SHA-256 (`4C:74:24:48:38:34:6A:D3:F8:BF:6D:1A:A3:CC:DE:CA:64:0D:18:A4:6E:77:BB:4F:3A:6B:0D:47:F8:B3:06:C6` — used to sign the device-test APK actually installable today) and the local upload/release keystore's SHA-256 (`DD:4B:A3:82:02:06:B6:A5:0D:32:92:02:05:39:5F:DF:25:F7:8E:3B:7E:A1:D0:0E:22:0F:EE:75:A8:C3:1A:95` — same certificate that signed `app-release.aab`, kept for continuity once a release-signed test APK exists). Neither is the future Play App Signing certificate. `ANDROID_APPLICATION_ID` is unset in production and the route falls back to its `co.elyio.app` default, which matches `twa-manifest.json`/`app/build.gradle`.
- Live verification performed from this machine after deploy (commands and raw output kept in session log, not reproduced here): both `https://www.elyio.co/.well-known/assetlinks.json` and `https://elyio.co/.well-known/assetlinks.json` return HTTP 200, `Content-Type: application/json`, no redirect, and the exact array above with `package_name: "co.elyio.app"`. `https://www.elyio.co/visit` returns 200. A headless Playwright session against the live site confirmed: no Google/PostHog network request before consent; clicking Accept attaches exactly one `#elyio-ga4` tag and sends `gtag('consent','update',{analytics_storage:'granted',...})`; the new "Privacy / Analytics settings" button reopens the choice; declining after Accept sends `analytics_storage:'denied'` immediately; PostHog (`us-assets.i.posthog.com`) only requests after consent is granted, never before; scanner/camera UI renders and is clickable under denied consent. `node scripts/pwa-runtime-check.mjs` against live production reports `controller:false`, `registrationCount:0`, `elyioCacheCount:0`. No production database mutation, no `/v1/recognize` call and no OpenAI cost were made from this session (see Recognition below).
- **Recognition smoke (task section 12) was intentionally NOT executed against production.** Backend/recognition code is untouched by this deploy. A real cross-check would require either (a) a paid `/v1/recognize` call, which without the Fly-only `ANALYTICS_QA_TOKEN`/`X-ELYIO-QA-Token` secret (not present in this repo/environment) would pollute founder metrics, contrary to the `AGENTS.md` invariant that QA/internal traffic must never enter founder metrics, or (b) a physical device with a camera, which was not available. Frontend recognition-rendering regressions (`test:museum-context`, `test:ai-result`) were run against this exact deployed frontend code and passed; that is the extent of recognition verification actually performed here.
- Device-test APK remains the existing debug artifact `android/app/build/outputs/apk/debug/app-debug.apk` (SHA-256 above), per an explicit decision in this session to not reconstruct the release keystore password (`ELYIO_RELEASE_STORE_PASSWORD`/`ELYIO_RELEASE_KEY_PASSWORD`, set only in the interrupted prior shell session and not persisted anywhere). `app-release.aab` remains signed with the local upload certificate and is not re-signed as an installable APK in this session.
- A2.5 gate status: web prerequisites are deployed and live-verified as above. `PHYSICAL_ANDROID_TEST` remains `NOT VERIFIED` — no real device was used in this session. Do not treat any statement above as camera/device proof.
