...
The frozen source snapshot reconciles to 3,785 holdings: 2,000 controlled image-backed works, 566 remaining image relationships, 6 explicit technical exclusions, and 1,219 metadata-only holdings. The restored canonical image pipeline produced 559 valid new references; one additional media PID returned HTTP 403, so the prepared valid package is 2,559 works. Controlled activation remains at 2,000 until the OpenAI-backed recognition and safety gates pass. Metadata-only holdings remain inactive.
