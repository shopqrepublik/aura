...
Quality-recovery diagnostics add `national_gallery_reference_asset_audit.py`, `national_gallery_population_quality_audit.py`, and `national_gallery_visual_retrieval_probe.py`. The production benchmark runner can record stage timings and candidate ranks with `--profile-stages --diagnose-retrieval`. Retrieval recall@1/3/5/10/20 must be reported separately from final verified recognition: a descriptor hit is never a successful recognition. Current runtime version `ng-controlled-500-v2-retrieval` preserves the same 500 works and thresholds while bounding expensive verification to one call with at most three references.
