...
The National Gallery controlled runtime is now `ng-controlled-1000-v1-retrieval`: 1,000 scoped candidates still produce at most five fused candidates, three reference images and one bounded reference-verifier call. A verifier `NEEDS_CONFIRMATION` decision is capped below the unchanged auto threshold. When a metadata-only seed conflicts with another same-artist visual candidate, the generic safety rule also preserves confirmation instead of auto-acceptance. See [the dated 1,000-work benchmark](NATIONAL_GALLERY_CONTROLLED_1000_BENCHMARK_2026-08-24.md).
