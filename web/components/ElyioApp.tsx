...
export default function ElyioApp({
  ...
  const detection = useMuseumDetection({ enabled: directToScanner });
  ...
  }, [actions, detection.museum, directToScanner, state.visitStarted]);

  useEffect(() => {
    if (!directToScanner || !state.visitStarted || !detection.museum || detection.status !== "detected") return;
    if (state.museumId === detection.museum.id) return;
    actions.setMuseumContext(detection.museum.id, detection.museum.name, detection.museum.city || detection.museum.region || null);
  }, [actions, detection.museum, detection.status, directToScanner, state.museumId, state.visitStarted]);
