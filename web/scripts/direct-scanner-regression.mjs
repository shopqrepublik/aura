...
async function runCase(browser, name, viewport, userAgent, cameraAccepted, geolocationDetected) {
  ...
  // Museum resolution is background enrichment. The scanner must be usable
  // before GPS resolves and without forcing a museum choice.
  ...
  // A detection callback may resolve just after this first scanner assertion;
  // either null (still pending) or louvre is valid at this point.
  if (geolocationDetected) assert.ok(stored.state.museumId === null || stored.state.museumId === "louvre", `${name}: invalid museum initialization`);
  else assert.equal(stored.state.museumId, null, `${name}: unexpected museum initialization`);
  assert.equal(await page.getByText(/Locating|Localisation|正在定位/).count(), 0, `${name}: blocking location screen returned`);
  ...
  await page.reload({ waitUntil: "domcontentloaded" });
  await page.getByRole("button", { name: "Capture" }).waitFor();
  if (!geolocationDetected) assert.equal(stored.state.museumId, null, `${name}: unknown visit gained stale museum context`);
